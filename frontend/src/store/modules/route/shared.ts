 ...
 * @param routes
 */
export function isRouteExistByRouteName(routeName: string, routes: ElegantConstRoute[]) {
 ...
 * @param routeName
 */
function recursiveGetIsRouteExistByRouteName(route: ElegantConstRoute, routeName: string) {
...
export function transformMenuToSearchMenus(menus: App.Global.Menu[], treeMap: App.Global.Menu[] = []) {
  ...
  return menus.reduce<App.Global.Menu[]>((acc, cur) => {
    if (!cur.children) {
      acc.push(cur);
    }

...
async function loadTrend(): Promise<void> {
  ...
  } catch { window.$message?.error('加载血糖趋势失败') }
}
...
function initTrendChart(): void {
  ...
  const markPoints = trendData.value
    ...
    .filter((p): p is { coord: (string | number)[]; value: string } => p !== null);
    ...
    series: [{
      ...
      markPoint: markPoints.length > 0 ? {
        data: markPoints.map(p => ({
          ...
          symbol: 'circle',
          symbolSize: 10,
          itemStyle: { color: p.value === '高' ? '#f85149' : '#fa8c16' },
          label: { show: true, formatter: p.value, color: '#e6edf3', fontSize: 10 }
        }))
      } : undefined
    }]
  });

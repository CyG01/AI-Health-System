...
const historyColumns: DataTableColumns<CheckinRecord> = [
  ...
  },
  {
    ...
    width: 100,
    render(row) {
      const type: 'success' | 'warning' | 'default' = row.exerciseStatus === 2 ? 'success' : row.exerciseStatus === 1 ? 'warning' : 'default';
      return h(NTag, { size: 'small', type }, { default: () => statusLabel(row.exerciseStatus) });
    }
  },
  {
    title: '饮食',
    ...
    width: 100,
    render(row) {
      const type: 'success' | 'warning' | 'default' = row.dietStatus === 2 ? 'success' : row.dietStatus === 1 ? 'warning' : 'default';
      return h(NTag, { size: 'small', type }, { default: () => statusLabel(row.dietStatus) });
    }
  },
  {
    title: '体重(kg)',
...
async function handleSubmit() {
  ...
  try {
    const payload: Api.Checkin.CheckinParams = {};
    ...
    const { error } = await fetchSubmitCheckin(payload);
    if (error) {
      message.error('打卡失败');
      return;
    }
...
async function handleSupplementSubmit() {
  submitting.value = true;
  try {
    const payload: Api.Checkin.CheckinParams & { date: string; reason: string } = {
      planId: supplementForm.planId ?? undefined,
    ...
    const { error } = await fetchSupplementCheckin(payload);
    if (error) {
      message.error('补卡失败');
      return;
    }
...
async function loadHistory() {
  ...
  try {
    ...
    const { data, error } = await fetchGetCheckinPage(params);
    if (error || !data) {

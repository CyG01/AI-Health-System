  ...
  <div class="p-1">
    <n-spin :show="pageLoading">
      ...
      <n-card class="max-w-[860px]">
        ...
        <!-- Summary Header -->
        <div class="flex items-center gap-4 mb-5 p-3 rounded-lg sport-summary-bg">
          <div class="flex items-center gap-2">
            <span class="text-2xl font-bold sport-text-secondary">{{ totalCount }}</span>
            <span class="text-xs sport-text-tertiary">总计划</span>
          </div>
          <span class="w-px h-8 sport-divider-bar"></span>
          <div class="flex items-center gap-2">
            <span class="text-2xl font-bold sport-color-warning">{{ activeCount }}</span>
            <span class="text-xs sport-text-tertiary">生效中</span>
          </div>
          <span class="w-px h-8 sport-divider-bar"></span>
          <div class="flex items-center gap-2">
            <span class="text-2xl font-bold sport-color-primary">{{ completedCount }}</span>
            <span class="text-xs sport-text-tertiary">已完成</span>
          </div>
        </div>
        ...
        <div v-if="list.length > 0" class="flex flex-col gap-3">
          <div
            v-for="plan in list"
            :key="plan.id"
            class="flex items-center justify-between p-4 rounded-lg cursor-pointer transition-all sport-plan-card"
            :class="[
              plan.status === 1
                ? 'sport-plan-active'
                : 'sport-plan-inactive'
          ...
          >
            <div class="flex flex-col gap-2">
              ...
              <div class="text-xs sport-text-tertiary">
                <span>{{ plan.durationDays }}天计划</span>
                <span class="mx-1 sport-text-secondary">|</span>
...
.sport-plan-inactive {
  border-color: var(--sport-border-subtle);
  background: var(--sport-bg-elevated);
}
</style>

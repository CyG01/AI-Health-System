        ...
        <n-card class="mt-5">
          ...
          <n-form :model="feedbackForm" label-placement="left" label-width="80">
            <n-form-item label="评分">
              <n-rate v-model:value="feedbackForm.satisfactionScore" :count="5" />
            </n-form-item>
              ...
              >
                <div class="flex items-center justify-between mb-2">
                  <n-space align="center" :size="8">
                    <n-rate :value="fb.satisfactionScore" :count="5" readonly size="small" />
                    <n-tag size="small" :type="fb.satisfactionScore >= 4 ? 'success' : fb.satisfactionScore >= 3 ? 'warning' : 'error'">
                      {{ fb.satisfactionScore }}分
                    </n-tag>
                  </n-space>
                  <span class="text-xs text-gray-500">{{ fb.createTime }}</span>
                </div>
...
// Feedback state
const feedbackForm = ref({ satisfactionScore: 0, content: '' });
...
const planDays = computed(() => planStore.currentPlanDays as unknown as PlanDay[]);
...
async function handleCompleteTask(dayIdx: number, itemIdx: number, item: string | StructuredItem): Promise<void> {
  ...
  try {
    ...
    planStore.updateDayItem(dayIdx, { completed: true });
...
let adjustStreamControl: { promise: Promise<void>; abort: () => void } | null = null;
...
async function handleAdjustPlan(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (data) {
        adjustResult.value = {
          summary: data.text || data.summary || '调整建议已生成',
          changes: data.widgets?.map(w => ({
            type: w.type || 'adjust',
...
async function handleSubmitFeedback(): Promise<void> {
  if (!feedbackForm.value.satisfactionScore) {
  ...
  try {
    await fetchSubmitPlanFeedback({
      ...
      satisfactionScore: feedbackForm.value.satisfactionScore,
...
async function handleSavePlanContent(): Promise<void> {
  ...
  try {
    ...
    if (planData?.aiContent) {
      ...
      } catch { window.$message?.error('计划内容解析失败') }
    }
    if (planData?.days && days.length === 0) {
      days.push(...planData.days.map(d => ({
        d: d.day || d.d || 0,
        items: d.tasks?.map(t => t.description || t) || d.items || []
      })));
    }
    planStore.setPlan(planData, days as unknown as Array<Record<string, unknown>>);
  } catch {

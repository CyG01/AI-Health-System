    ...
    <NCard v-if="selectedItem" :bordered="false">
      ...
      <div v-if="guidance" class="flex flex-col gap-4">
        <div v-if="guidance.basicInfo">
          <h4 class="mb-2 border-b pb-1 text-sm text-[var(--sport-secondary)]" :style="{ borderColor: 'var(--sport-border)' }">基本信息</h4>
        ...
        <div v-if="guidance.breathing">
          <h4 class="mb-2 border-b pb-1 text-sm text-[var(--sport-secondary)]" :style="{ borderColor: 'var(--sport-border)' }">呼吸节奏</h4>
          <p class="text-[13px] leading-relaxed">{{ guidance.breathing }}</p>
        </div>
        <div v-if="guidance.steps?.length">
          <h4 class="mb-2 border-b pb-1 text-sm text-[var(--sport-secondary)]" :style="{ borderColor: 'var(--sport-border)' }">动作要领</h4>
          <ul class="flex flex-col gap-2">
            <li v-for="(step, i) in guidance.steps" :key="i" class="flex items-start gap-2.5 text-[13px] leading-relaxed">
              <span class="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#FF6B35] to-[#FF6B6B] text-[11px] font-bold text-white">{{ Number(i) + 1 }}</span>
              {{ step }}
            </li>
          </ul>
        </div>
        <div v-if="guidance.commonMistakes?.length">
          <h4 class="mb-2 border-b pb-1 text-sm text-[var(--sport-secondary)]" :style="{ borderColor: 'var(--sport-border)' }">常见错误</h4>
          <ul class="flex flex-col gap-2">
        ...
        <div v-if="guidance.tips">
          <h4 class="mb-2 border-b pb-1 text-sm text-[var(--sport-secondary)]" :style="{ borderColor: 'var(--sport-border)' }">小贴士</h4>
...
async function handleSubmit() {
  ...
  try {
    ...
    const { error } = await fetchSubmitExerciseRecord({
      itemId: form.value.itemId ?? undefined,

  ...
  <div class="food-record-page flex flex-col gap-5">
    ...
    <NCard :title="$t('food.aiRecognition') || 'AI智能食物识别'" :bordered="false">
      <div class="flex flex-col gap-4">
        ...
        <!-- Recognition Result -->
        <div v-if="recognitionResult" class="rounded-lg border p-4" :style="{ borderColor: 'color-mix(in srgb, var(--sport-primary) 15%, transparent)', background: 'color-mix(in srgb, var(--sport-primary) 6%, transparent)' }">
          <div class="mb-3 flex items-center justify-between">
            <h3 class="text-[15px] text-[#4ECDC4]">识别结果</h3>
            <NButton text size="small" @click="recognitionResult = null; uploadedImage = ''">清除</NButton>
          </div>
          <div class="flex items-center gap-2.5 rounded-lg border bg-[var(--sport-bg-elevated)] p-2.5" :style="{ borderColor: 'var(--sport-border)' }">
    ...
    <NCard :title="$t('food.nlpInput') || '一句话记账'" :bordered="false">
      <div class="flex flex-col gap-3">
        ...
        <!-- NLP Result Preview -->
        <div v-if="nlpResult" class="rounded-lg border p-3" :style="{ borderColor: 'color-mix(in srgb, var(--sport-primary) 15%, transparent)', background: 'color-mix(in srgb, var(--sport-primary) 6%, transparent)' }">
          <div
            v-for="(item, i) in nlpResult.items"
            :key="i"
            class="mb-2 flex items-center gap-2.5 rounded-lg border bg-[var(--sport-bg-elevated)] p-2.5 text-[13px]" :style="{ borderColor: 'var(--sport-border)' }"
          >
            ...
            <span class="whitespace-nowrap text-xs text-[#FFB800]">≈ {{ item.calories }} kcal</span>
            <NButton size="small" type="error" text @click="removeNlpItem(i)">删除</NButton>
          </div>
          <div class="flex justify-end gap-2.5 border-t pt-2" :style="{ borderColor: 'var(--sport-border)' }">
...
async function handleEditSubmit() {
  ...
  try {
    const { error } = await fetchUpdateFoodRecord(editingId.value, {
      itemId: editForm.value.itemId ?? undefined,
...
const tableColumns: DataTableColumns = [
  { title: '食物', key: 'foodItemName', minWidth: 120 },
  {
    ...
    render(row: any) {
      const type: 'default' | 'success' | 'warning' | 'info' = row.mealType === 'breakfast' ? 'default' : row.mealType === 'lunch' ? 'success' : row.mealType === 'dinner' ? 'warning' : 'info';
      return h(NTag, { size: 'small', type }, { default: () => mealTypeMap[row.mealType] || row.mealType });
    }
  },
    ...
    for (const item of nlpResult.value.items) {
      ...
      if (matched) {
        ...
        await fetchSubmitFoodRecord({
          itemId: matched.id, weightGrams: weightG, caloriesConsumed, checkinId: checkinId ?? undefined,
...
async function handleSubmit() {
  ...
  try {
    ...
    const { error } = await fetchSubmitFoodRecord({
      itemId: form.value.itemId ?? undefined, weightGrams: form.value.weightGrams,
      caloriesConsumed, checkinId: checkinId ?? undefined, mealType: form.value.mealType,

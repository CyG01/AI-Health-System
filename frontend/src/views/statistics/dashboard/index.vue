      ...
      <NCard :bordered="false" class="progress-bar-card">
        <div class="flex flex-wrap items-center justify-center gap-6">
          <div class="flex flex-col items-center gap-1">
            <span class="text-2xl font-bold font-tabular text-[#FF6B35]">{{ statsProgress.totalCheckinRate }}%</span>
            <span class="text-xs text-secondary">{{ $t('stats.totalCheckinRate') || '总完成率' }}</span>
          </div>
          <div class="h-9 w-px bg-[var(--sport-border)]" />
          <div class="flex flex-col items-center gap-1">
            <span class="text-2xl font-bold font-tabular text-[#4ECDC4]">{{ statsProgress.exerciseCompleteRate }}%</span>
            <span class="text-xs text-secondary">{{ $t('stats.exerciseCompleteRate') || '运动完成率' }}</span>
          </div>
          <div class="h-9 w-px bg-[var(--sport-border)]" />
          <div class="flex flex-col items-center gap-1">
            <span class="text-2xl font-bold font-tabular text-[#FFB800]">{{ statsProgress.dietCompleteRate }}%</span>
              ...
              <NCard class="section-card">
                <div class="plan-progress">
                  ...
                  <NProgress
                    ...
                    :color="progressColor"
                    :rail-color="'var(--sport-border-subtle)'"
                  />
                  <div class="plan-hint">
          ...
          <template v-if="weekData">
            ...
            <NCard :title="$t('page.dashboard.dietComparisonWeek') || '饮食热量周对比'" class="chart-card">
              <template #header-extra>
                <span v-if="dietComparison" class="text-xs text-[var(--sport-text-tertiary)]">
        ...
        <div class="stats-charts-grid">
          ...
          <div class="stats-chart-item full-width">
            <NCard :bordered="false" :title="$t('stats.weightTrend') || '体重变化趋势'">
              ...
              <div v-else class="flex min-h-[220px] items-center justify-center gap-3 text-[#FF6B6B]">
            ...
            <template v-if="onProgress">
              <div class="progress-content">
                <div class="progress-item">
                  ...
                  <NProgress type="line" :percentage="progressPercent" :color="progressColor" :rail-color="'var(--sport-border-subtle)'" />
                </div>
...
const CC = { green: '#FF6B35', blue: '#4ECDC4', amber: '#FFB800' };
const AX = { textStyle: { color: 'var(--sport-text-tertiary)' }, axisLine: { lineStyle: { color: 'var(--sport-border)' } }, splitLine: { lineStyle: { color: 'var(--sport-border)' } } };
...
const weightChangeClass = computed(() => {
  ...
  return v > 0 ? 'text-[#FF6B6B]' : 'text-[#FF6B35]';
});
...
function initDietComparisonChart() {
  ...
  dietCompChart.setOption({
    tooltip: { trigger: 'axis' },
    legend: { data: [data.currentPeriodLabel as string, data.previousPeriodLabel as string], textStyle: { color: 'var(--sport-text-tertiary)' } },
    grid: { left: 50, right: 20, top: 40, bottom: 30 },
    xAxis: { type: 'category', data: xLabels, axisLabel: { color: 'var(--sport-text-tertiary)' } },
    yAxis: { type: 'value', name: 'kcal', axisLabel: { color: 'var(--sport-text-tertiary)' } },
    series: [
      {
      ...
      },
      {
        ...
        lineStyle: { color: 'var(--sport-text-tertiary)', width: 2, type: 'dashed' },
        itemStyle: { color: 'var(--sport-text-tertiary)' },
        areaStyle: {
          color: {
            type: 'linear', x: 0, y: 0, x2: 0, y2: 1,
            colorStops: [
...
function dt() {
  return { backgroundColor: 'var(--sport-bg-surface)', borderColor: 'var(--sport-border)', textStyle: { color: 'var(--sport-text-primary)', fontSize: 12 } };
}
...
function buildLineChart(el: HTMLDivElement | null, x: string[], y: number[], unit: string, color: string) {
  ...
  c.setOption({
    ...
    yAxis: { type: 'value', name: unit, nameTextStyle: { color: 'var(--sport-text-tertiary)', fontSize: 11 }, ...AX },
    series: [{
...
function buildBarChart(el: HTMLDivElement | null, x: string[], y: number[], unit: string, color: string) {
  ...
  c.setOption({
    ...
    grid: { top: 12, right: 20, bottom: 28, left: 48 },
    xAxis: { type: 'category', data: x, ...AX },
    yAxis: { type: 'value', name: unit, nameTextStyle: { color: 'var(--sport-text-tertiary)', fontSize: 11 }, ...AX, min: 0, max: 100 },
    series: [{
...
function buildDeficitChart(el: HTMLDivElement | null, x: string[], con: number[], bur: number[], net: number[]) {
  ...
  c.setOption({
    tooltip: { ...dt(), trigger: 'axis' },
    legend: { data: ['摄入', '消耗', '净差值'], textStyle: { color: 'var(--sport-text-tertiary)', fontSize: 11 }, top: 0 },
    grid: { top: 32, right: 20, bottom: 28, left: 48 },
    xAxis: { type: 'category', data: x, ...AX },
    yAxis: { type: 'value', name: 'kcal', ...AX },
    series: [
      { name: '摄入', type: 'bar', data: con, itemStyle: { color: '#FF6B6B', borderRadius: [4, 4, 0, 0] } },
      { name: '消耗', type: 'bar', data: bur, itemStyle: { color: '#4ECDC4', borderRadius: [4, 4, 0, 0] } },
      { name: '净差值', type: 'line', data: net, smooth: true, lineStyle: { color: '#FF6B35', width: 2 }, itemStyle: { color: '#FF6B35' } }
    ]
  });
}
...
function buildPieChart(el: HTMLDivElement | null, names: string[], values: number[], title: string) {
  ...
  c.setOption({
    ...
    legend: { orient: 'vertical', right: 8, top: 'center', textStyle: { color: 'var(--sport-text-tertiary)', fontSize: 11 } },
    series: [{
      name: title, type: 'pie', radius: ['40%', '70%'], center: ['35%', '50%'],
      itemStyle: { borderRadius: 4, borderColor: 'var(--sport-bg-surface)', borderWidth: 2 },
      label: { show: false },
...
async function handleExport(key: string) {
  const exportFn = key === 'excel' ? fetchExportExcel : fetchExportCSV;
  const ext = key === 'excel' ? 'xlsx' : 'csv';
  ...
  try {
    ...
    link.href = url;
    link.download = `health-data-${new Date().toISOString().split('T')[0]}.${ext}`;
...
.card-reminder {
  background: var(--sport-accent-warm) !important;
  color: #fff;
}
.card-celebration {
...
.text-secondary { color: var(--n-text-color-3, var(--sport-text-tertiary)); }
...
.plan-hint { font-size: 12px; color: var(--sport-text-tertiary); margin-top: 6px; }
...
.detail-item { font-size: 13px; color: var(--sport-text-tertiary); }
.detail-item strong { color: var(--sport-text-primary); }
...
.recommend-item .item-meta { font-size: 12px; color: var(--sport-text-tertiary); }
...
/* AI Suggestions */
.ai-suggestion-card { border-left: 3px solid var(--sport-secondary); }
.ai-suggestion-text { color: var(--sport-text-secondary); font-size: 14px; line-height: 1.8; margin: 0; }

...
/** Check if feedback is pending (not yet reviewed) */
function isPending(item: AiFeedbackItem): boolean {
  return item.manualReviewed === 0;
}
...
const filteredItems = computed(() => {
  return items.value.filter(item => {
    ...
    if (query.reviewResult) {
      if (query.reviewResult === 'pending') {
        matchStatus = isPending(item);
      } else {
        matchStatus = item.reviewResult === query.reviewResult;
      }
    }
...
function reviewStatusLabel(s: string): string {
  const map: Record<string, string> = { valid: '有效', invalid: '无效', duplicate: '重复' };
  return map[s] || s || '待审核';
}
...
const columns: DataTableColumns<AiFeedbackItem> = [
  ...
  },
  {
    ...
    width: 100,
    render(row) {
      const status = isPending(row) ? '' : row.reviewResult;
      return h(
        NTag,
        { type: reviewStatusTagType(status), size: 'small', bordered: false },
        { default: () => reviewStatusLabel(status) }
      );
    }
  },
  {
...
async function loadItems() {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      data = data.filter((i: AiFeedbackItem) => isPending(i));
    }
    items.value = data;
  } finally {
    loading.value = false;
    ...
    <NCard title="AI反馈审核">
      <template #header-extra>
        <NSpace align="center">
          ...
          />
          <NSelect
            v-model:value="query.reviewResult"

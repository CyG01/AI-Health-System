...
async function fetchData() {
  ...
  try {
    ...
    if (res.data) {
      tableData.value = (res.data.records || []) as unknown as UserRow[];
...
async function handleExportUsers() {
  ...
  try {
    ...
    if (data && Array.isArray(data)) {
      const header = ['ID', '用户名', '手机号', '角色', '状态', '注册时间', '最后登录'];
      const rows = data.map((u: any) => [
        u.id, u.username, u.phone || '', u.role, u.status === 1 ? '正常' : '禁用', u.createTime || '', u.lastLoginTime || ''
      ]);
      ...
      <NSpin :show="detailLoading">
        <div v-if="userDetail">
          ...
          <NDescriptions label-placement="left" bordered :column="2" size="small">
            <NDescriptionsItem label="用户ID">{{ userDetail.id }}</NDescriptionsItem>
            ...
            <NDescriptionsItem label="注册时间">{{ userDetail.createTime || '-' }}</NDescriptionsItem>
            <NDescriptionsItem label="最后登录">{{ userDetail.lastLoginTime || '-' }}</NDescriptionsItem>
            <NDescriptionsItem label="状态">
              <NTag :type="userDetail.status === 1 ? 'success' : 'error'" size="small">
                {{ userDetail.status === 1 ? '正常' : '禁用' }}
              </NTag>
            </NDescriptionsItem>
          </NDescriptions>
          <NDivider style="margin: 16px 0 12px" />
          <h4 class="text-sm font-semibold mb-2">健康档案</h4>
          <NDescriptions label-placement="left" bordered :column="2" size="small">
            <NDescriptionsItem label="身高">{{ userDetail.height ? userDetail.height + ' cm' : '-' }}</NDescriptionsItem>
            <NDescriptionsItem label="体重">{{ userDetail.weight ? userDetail.weight + ' kg' : '-' }}</NDescriptionsItem>
            <NDescriptionsItem label="年龄">{{ userDetail.age || '-' }}</NDescriptionsItem>
            <NDescriptionsItem label="BMI等级">{{ userDetail.bmiLevel || '-' }}</NDescriptionsItem>
          </NDescriptions>
          <NDivider style="margin: 16px 0 12px" />
          <h4 class="text-sm font-semibold mb-2">统计概览</h4>
          <NDescriptions label-placement="left" bordered :column="2" size="small">
            <NDescriptionsItem label="计划总数">{{ userDetail.totalPlans }}</NDescriptionsItem>
            <NDescriptionsItem label="活跃计划">{{ userDetail.activePlanCount }}</NDescriptionsItem>
            <NDescriptionsItem label="打卡天数">{{ userDetail.totalCheckinDays }}</NDescriptionsItem>
            <NDescriptionsItem label="连续打卡">{{ userDetail.consecutiveDays }}</NDescriptionsItem>
            <NDescriptionsItem label="运动记录">{{ userDetail.totalExerciseRecords }} 条</NDescriptionsItem>
            <NDescriptionsItem label="运动总消耗">{{ userDetail.totalExerciseCalories }} kcal</NDescriptionsItem>
            <NDescriptionsItem label="饮食记录">{{ userDetail.totalDietRecords }} 条</NDescriptionsItem>
            <NDescriptionsItem label="饮食总摄入">{{ userDetail.totalDietCalories }} kcal</NDescriptionsItem>
          </NDescriptions>
        </div>

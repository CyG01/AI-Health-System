...
type BodyRecord = Api.Health.BodyMeasurement;
...
async function loadHistory() {
  ...
  try {
    const { data } = await fetchGetBodyMeasurementHistory(HISTORY_PAGE_SIZE)
    history.value = data || []
  } finally {
...
async function loadTrend() {
  try {
    const { data } = await fetchGetBodyMeasurementTrend(TREND_MONTHS)
    trend.value = data || []
  } catch { message.error('加载围度趋势失败') }
}
...
async function handleSubmit() {
  ...
  try {
    await fetchSubmitBodyMeasurement({ ...form })
      ...
      series: [
        {
          ...
          markLine: {
            data: [
              { yAxis: 0.85, label: { formatter: '健康线 0.85' }, lineStyle: { color: '#FF6B35' } },
...
.text-success { color: var(--sport-primary); }

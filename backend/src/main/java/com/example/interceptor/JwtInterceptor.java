...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (token == null || token.isBlank()) {
            throw new BusinessException(Result.UNAUTHORIZED_CODE, "未登录或token已过期");
        }
        if (!jwtUtil.validateToken(token)) {
            // token验证失败，区分是过期还是其他原因
            if (jwtUtil.isTokenExpired(token)) {
                throw new BusinessException(Result.TOKEN_EXPIRED_CODE, "token已过期，请刷新token");
            }
            throw new BusinessException(Result.UNAUTHORIZED_CODE, "无效的token");
        }
        if (!jwtUtil.isAccessToken(token)) {
            throw new BusinessException(Result.UNAUTHORIZED_CODE, "请使用access token访问");
        }
        ...
        if (Boolean.TRUE.equals(isBlacklisted)) {
            throw new BusinessException(Result.UNAUTHORIZED_CODE, "token已失效，请重新登录");
        }

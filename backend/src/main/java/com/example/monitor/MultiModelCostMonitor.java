...
public class MultiModelCostMonitor {
    ...
    @Value("${cost.budget.daily-global:10.00}")
    private BigDecimal globalDailyBudget;
    ...
    /**
     * 简化版记录（兼容旧调用，默认 HIGH 层级）。
     */
    public void recordCall(int inputTokens, int outputTokens) {
        recordCall(inputTokens, outputTokens, ModelTier.HIGH);
    }
    ...
    public BigDecimal getRemainingBudget() {
        BigDecimal remaining = globalDailyBudget.subtract(getGlobalDailyCost());
        return remaining.compareTo(BigDecimal.ZERO) > 0 ? remaining : BigDecimal.ZERO;
    }

    /**
     * 获取全局当日总成本（别名方法，兼容原 DeepSeekCostMonitor 调用方）。
     */
    public BigDecimal getCurrentDailyCost() {
        return getGlobalDailyCost();
    }
    ...
    public boolean isGlobalCostExceeded() {
        return getGlobalDailyCost().compareTo(globalDailyBudget) >= 0;
    }

    ...
    public ExerciseGuidanceVO getGuidance(Long exerciseId) {
        ...
        if (cached != null) {
            try {
                ...
                log.debug("Failed to deserialize exercise guidance from cache: {}", e.getMessage());
            }
        }
        ...
        if (item.getAiGuidance() != null && !item.getAiGuidance().isEmpty()) {
            try {
                ...
                log.debug("Failed to parse AI guidance from DB for exerciseId={}: {}", exerciseId, e.getMessage());
            }
        }

...
public class AutoPlanAdjustService {
     ...
     */
    @Async("planAdjustExecutor")
    public void adjustForTravelScenario(Long userId) {
        AiPlan activePlan = getActiveSportPlan(userId);
        if (activePlan == null) return;

        log.info("出差场景自动调整计划 userId={} planId={}", userId, activePlan.getId());

        HealthRecordVO health = healthService.getLatestHealthRecord(userId);
        String prompt = String.format(
                "用户当前处于出差场景，无法使用健身房和器械。当前运动计划：%s\n" +
                        "用户BMI=%.1f，健康目标：%s。\n" +
                        "请自动调整运动计划为酒店室内无器械方案：\n" +
                        "1. 所有动作不使用任何器械，仅利用自重\n" +
                        "2. 适合酒店房间等有限空间\n" +
        ...
        try {
            String adjustedContent = modelRouter.singleChat(
                    "你是专业健康教练，为出差用户生成酒店室内无器械运动计划。输出结构化JSON。",
                    prompt, "plan_generate");

            applyPlanAdjustment(activePlan, userId, adjustedContent, "TRAVEL_ADJUST");
            notifyUser(userId, "TRAVEL_ADJUST",
                    "出差模式计划已调整",
                    "已为你生成酒店室内无器械运动方案，无需健身房也能保持锻炼节奏。");
        } catch (Exception e) {
            log.error("出差场景自动调整失败 userId={}", userId, e);
        }
    }
